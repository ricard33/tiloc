...
LOGGING = {
    ...
    "formatters": {
        "verbose": {"format": "%(name)-12s: %(asctime)s %(levelname)-8s [%(threadName)s] %(message)s"},
        "simple": {"format": "%(levelname)s %(message)s"},
        "json": {"()": "core.log_json_formatter.CustomisedJSONFormatter"},
    },
    ...
    "handlers": {
        ...
        "loggly": {
            ...
            % {"key": config.get("LOGGLY", "API_KEY", ""), "tag": config.get("LOGGLY", "TAG", "python")},
        },
        "mail_admins": {
            "level": "ERROR",
